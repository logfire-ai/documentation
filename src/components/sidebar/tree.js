...
function sortItems(items) {
  ...
  items.sort((a, b) => {
    ...
    // If both titles start with numbers, sort by number
    if (!isNaN(numA) && !isNaN(numB)) {
      return numA - numB;
    }
    // If only the first title starts with a number, it should come first
    else if (!isNaN(numA)) {
      return -1;
    }
    // If only the second title starts with a number, it should come first
    else if (!isNaN(numB)) {
      return 1;
    }
    // If neither title starts with a number, sort them alphabetically
    else {
